...
export default class Coupon extends React.Component {
    ...
    loadCoupon = async () => {
        ...
        try {
            let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/coupon/search", requestOptions)
            if (response.ok) {
    ...
    deleteCoupon = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa không ?')) {
            ...
            try {
                let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/coupon/"+id, requestOptions)
                if(response.ok) {
            ...
            <tbody>
            {
                this.props.data.map(item => {
                    return <tr key={item.id}>
                        ...
                        <td>{item.present}</td>
                        <td>
                            <button type="button" ><Link to={"/admin/coupon/edit/"+item.id}>Sửa</Link></button>
