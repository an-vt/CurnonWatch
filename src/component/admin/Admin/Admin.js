...
export default class Admin extends React.Component {
    ...
    loadAdmin = async () => {
        ...
        try {
            let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/account/search", requestOptions)
            if (response.ok) {
    ...
    deleteAdmin = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa không ?')) {
            ...
            try {
                let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/account/"+id, requestOptions)
                if(response.ok) {
            ...
            <tbody>
            {
                this.props.data.map(item => {
                    return <tr key={item.id}>
                        ...
                        <td>
                            <button type="button" ><Link to={"/admin/account/edit/"+item.id}>Sửa</Link></button>
