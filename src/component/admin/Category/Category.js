...
export default class Category extends React.Component {
    ...
    loadCategory = async () => {
        ...
        try {
            let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/category/search", requestOptions)
            if (response.ok) {
    ...
    deleteCategory = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa không ?')) {
            ...
            try {
                let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/category/"+id, requestOptions)
                if(response.ok) {
            ...
            <tbody>
                {
                    this.props.data.map(item => {
                        return <tr key={item.id}>
                            ...
                            <td>{item.name}</td>
                            <td>
                                <button type="button" ><Link to={"/admin/category/edit/" + item.id}>Sửa</Link></button>
