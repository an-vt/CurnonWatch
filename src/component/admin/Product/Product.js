...
export default class Product extends React.Component {
    ...
    loadProduct = async () => {
        ...
        try {
            let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/product/search", requestOptions)
            if (response.ok) {
    ...
    deleteProduct = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa không ?')) {
            ...
            try {
                let response = await fetch("https://api-curnon-springbooot.herokuapp.com/api/admin/product/"+id, requestOptions)
                if(response.ok) {
            ...
            <tbody>
            {
                this.props.data.map(item => {
                    return <tr key={item.id}>
                        ...
                        <td><img width="80px" src={"https://api-curnon-springbooot.herokuapp.com/api/download/"+item.image} /></td>
                        <td>{item.categoryDTO.name}</td>
                        <td>
                            <button type="button" ><Link to={"/admin/product/edit/"+item.id}>Sửa</Link></button>
